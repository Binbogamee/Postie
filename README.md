...
Функция сервиса не требует токена аутентификации.
...
- **Redis:** используется для кэширования jwt токенов
...
1. Создайте файл .env, в котором прописаны значения необходимых переменных окружения.
    ...
    APIGATEWAY_PORT=5000 # порт, по которому будет доступен api gateway
    ACCOUNT_SERVICE_PORT=5001 # порт, по которому будет доступен account service
    AUTH_SERVICE_PORT=5002 # порт, по которому будет доступен auth service
    LOGGING_SERVICE_PORT=5003 # порт, по которому будет доступен logging service
    POST_SERVICE_PORT=5004 # порт, по которому будет доступен post service
